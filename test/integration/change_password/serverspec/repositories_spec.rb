...
-f $base64AuthInfo)} #{base_uri}/get_repo/run -Method POST #{textplain} -Body 'doesnotexist'"
    ...
    describe command('curl -uadmin:newPassword http://localhost:8081/service/rest/v1/script/foo/run ' \
                     '-X POST -H "Content-Type: text/plain"') do
      its(:exit_status) { should eq 0 }
    end

    describe command('curl -uadmin:newPassword http://localhost:8081/service/rest/v1/script/get_repo/run ' \
