...
describe 'nexus::default' do
  if os[:family] == 'windows'
    ...
    ping = <<-EOF
$base64AuthInfo = [Convert]::ToBase64String([Text.Encoding]::ASCII.GetBytes(("{0}:{1}" -f 'admin','newPassword'))); \
...
EOH
    ...
    describe command('curl -u admin:newPassword http://localhost:8081/service/metrics/ping') do
      its(:stdout) { should contain('pong') }
    end

    describe command('curl -u admin:newPassword http://localhost:8081/service/rest/v1/script/foo') do
