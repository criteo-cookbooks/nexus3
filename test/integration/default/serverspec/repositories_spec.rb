...
-f $base64AuthInfo)} #{base_uri}/get_repo/run -Method POST #{textplain} -Body 'doesnotexist'"
    ...
    describe command('curl -uadmin:admin456 http://localhost:8081/service/siesta/rest/v1/script -X POST ' \
                     '-H "Content-Type: application/json" -d \'{"name":"get_repo","type":"groovy",' \
                     '"content":"repository.repositoryManager.get(args)"}\'') do
      its(:exit_status) { should eq 0 }
    end

    describe command('curl -uadmin:admin456 http://localhost:8081/service/siesta/rest/v1/script/foo/run ' \
                     '-X POST -H "Content-Type: text/plain"') do
      its(:exit_status) { should eq 0 }
    end

    describe command('curl -uadmin:admin456 http://localhost:8081/service/siesta/rest/v1/script/get_repo/run ' \
